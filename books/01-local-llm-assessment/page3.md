...
- その分割したテキスト（チャンク：意味のある小さな塊）毎に「**要点生成用テンプレート**」を用意します。
- 各チャンク毎に生成した要点をセットできる「**概要生成用テンプレート**」を用意します。
...
- 出力パーサーの1つである「**StrOutputParser**」は生成結果を文字列だけ返すように指示します。（meta情報も除かれます）
...
final_summary = chain02.invoke(
  {"summaries": combined}, #　←各チャンク毎に要点を結合した値
